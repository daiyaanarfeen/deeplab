...
class VOCDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, aug=False):
        self.root = root
        ...
        for name in self.img_ids:
            ...
            if aug:
                ...
                label_file = osp.join(self.root, "SegmentationClassRaw/%s.png" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
    ...
    def __getitem__(self, index):
        ...
        if pad_h > 0 or pad_w > 0:
            image = cv2.copyMakeBorder(image, 0, pad_h, 0, 
                ...
                value=(0.0, 0.0, 0.0))
        image = image.transpose((2, 0, 1))
        F.normalize(image, [.485, .456, .406], [.229, .224, .225])
